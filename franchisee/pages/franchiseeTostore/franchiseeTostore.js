...
Page({
  ...
  onLoad: function(options){
    ...
    this.getAppShopConfig(); // 获取店铺配置情况
  ...
  getTostoreWaitingRule: function () {
    ...
    app.sendRequest({
      ...
      success: function (res) {
        ...
        if (res.data.fields_data && Number(res.data.fields_data.desc_show_type) === 1) {
          newdata['descIsRichText'] = true;
          WxParse.wxParse('wxParseDescription', 'html', res.data.description, that, 10); // 解析富文本
        }
        if (res.data.industry_type_name && (~res.data.industry_type_name.search(/\-/))) { // 设定一级行业分类名称
          res.data.industry_type_short_name = res.data.industry_type_name.split(/\-/)[0];
        }
        app.setPageTitle( res.data.title );
        newdata['topNavBarData.title'] = res.data.title;
        newdata['franchiseeInfo'] = res.data;
        that.setData(newdata);
  ...
  goodsListScroll: function(e){
    ...
    if (that.assessScrollFlag) {
      return;
    }
    that.assessScrollFlag = true;
    setTimeout(function () {
  ...
  clickChooseComplete: function(e){
    ...
    if (allow){
      ...
      app.turnToPage('/orderMeal/pages/previewOrderDetail/previewOrderDetail?cart_arr=' + cartId + '&franchisee=' + this.data.franchiseeId)
    }
  },
  ...
  onShareAppMessage: function (event) {
    ...
    let path = '/franchisee/pages/franchiseeTostore/franchiseeTostore?detail=' + that.data.franchiseeId;
    let image = '';
    let fromWhere = '';
