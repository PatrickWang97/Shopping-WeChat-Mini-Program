  ...
  methods: {
    onLoad: function (options) {
      ...
      this.setData({
        ...
        isBuyTop: false,
        topOrderInfo: {},
        top: pageInstance && (pageInstance.data.page_hasNavBar || pageInstance.data.topNavBarData && pageInstance.data.topNavBarData.isDefault == 0) ? app.globalData.topNavBarHeight : 0
      });
      if (articleId) {
    ...
    submitTimer: null,
    submitData: function (event, repeat) {
      let that = this,
      ...
      if (!title) {
        if (repeat === undefined) {
          this.submitTimer = setTimeout(() => this.submitData(event, true), 500);
          return;
        }
    ...
    afterCheckSubmitData: function (url, data) {
      let that = this;
      if (that.data.isShowTop && !that.data.isBuyTop){
        data.topTime = that.data.selectedTime[that.data.selectedTimeIndex].days;
        data.topPrice = that.data.selectedTime[that.data.selectedTimeIndex].price;
        app.sendRequest({
          url: '/index.php?r=AppSNS/AddSectionOrder',
          data: {
            days: that.data.selectedTime[that.data.selectedTimeIndex].days,
          ...
          success: function (res) {
            let param = res.data;
            param.orderId = res.order_id;
            param.success = function () {
              data.order_id = res.order_id;
              that.afterPaySubmitData(url, data);
            };
    ...
    afterPaySubmitData: function (url, data) {
      let that = this;
      app.sendRequest({
        url: url,
        data: data,
        ...
        success: function (res) {
          app.requestSubscribeMessage([{
            type: 64,
            obj_id: res.article_id,
          },{
            type: 2048,
            obj_id: res.article_id,
          }]).then(() => {
            let content = that.data.require_audit == 1 ? '提交成功，待审核。' : '提交成功';
            if (that.data.require_audit == 1 && data.topTime) {
              content = "提交成功，待审核。审核如果失败，置顶费用将原路返回";
            }
            app.showModal({
              content: content,
              complete: function() {
    ...
    getSections: function (communityId, callback) {
      ...
      app.sendRequest({
        ...
        success: function (res) {
          ...
          newdata.article_style = sec.article_style;
          ...
          if (communityId) {
            ...
            newdata.showTopWrap = res.data[0].price_set ? true : false;
            newdata.selectedTime = res.data[0].price_set ? res.data[0].price_set : {};
          }
          newdata['sectionList'] = res.data.filter((section) => {
            return section.require_article_post == 1;
          });
    ...
    getCategory: function (sectionId, callback) {
      ...
      app.sendRequest({
        ...
        success: function (res) {
          if (res.status == 0) {
            let info = res.data,
              newdata = [{ id: '', name: '默认全部分类' }].concat(info);
      ...
      app.sendRequest({
        ...
        success: function (res) {
          if (res.status == 0) {
            ...
            newdata['isBuyTop'] = !!info.is_paid_article;
            newdata['isShowTop'] = !!info.is_paid_article;
            newdata['topOrderInfo'] = info.order_info;
    ...
    selectSectionAct: function (e) {
      ...
      if (currentSection.require_article_post == 0) {
        app.showModal({
          content: '当前选中圈组不允许发布话题'
        });
        return;
      }
      this.setData({
        ...
        'showTopWrap': currentSection.price_set ? true : false,
        'selectedTime': currentSection.price_set || [],
        'communityId': currentSection.id,
        'selectedCategoryIndex': '',
        'selectedCategoryName': '',
        'showCategoryList': true,
        'article_style': currentSection.article_style
      });
    ...
    bindLinkInput: function (e) {
      ...
      this.setData({
        'articleData.origin_url': articlelinkInput,
        'articleData.view_origin_url': articlelinkInput.replace(/\\n/g, '\n')
      });
    },
    ...
    chooseTopTime: function (e) {
      this.setData({
        selectedTimeIndex: e.detail.value
      })
    },
    toSetingTop:  function (e) {
      this.setData({
        isShowTop: e.detail.value
      })
    },
    submitRegion: function (e) {
      this.setData({
        selectedTimeIndex: e.detail.value
      })
    }
