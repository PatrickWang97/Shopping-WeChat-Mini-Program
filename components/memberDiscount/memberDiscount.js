...
Component({
  ...
  methods: {
    ...
    showDialog: function (selectedDiscount, franchisee = '') {
      ...
      if (!discountType) {
        let discount_type = '';
        this.data.discountType.forEach((val) => {
      ...
      this.data.discountType.forEach((val) => {
        if (val.label === discountType || (val.label === 'vip' && discountType === 'paid_vip') || (val.label === 'vip' && discountType === 'vip_benefit_day')) {
          ...
          if (discountType === 'coupon') {
            ...
            currentTab = 'integral';
          } else if (discountType === 'times_card') {
            val.value.forEach((item, index) => {
              if (item.user_times_card_id == selectedDiscount.user_times_card_id) {
                currentDiscount = {
                  type: 'times_card',
                  index: index
                };
                currentTab = 'times_card';
              }
            })
          }
        }
    ...
    tipNameInit: function () {
      ...
      this.data.discountType.forEach((val) => {
        if (val.label === 'coupon' && val.value.length) {
          ...
          tipName.push('积分');
        } else if (val.label === 'times_card' && val.value.length) {
          tipName.push('次卡');
        }
      })
      this.setData({
    ...
    selectedDiscount: function (e) {
      ...
      this.data.discountType.forEach((val) => {
        if (val.label === type) {
          if (type === 'integral' && this.startFrom !== 'integral' && val.value[0].user_integral < val.value[0].max_can_use_integral) {
            app.showModal({
              content: '可用积分不足，不能使用积分优惠'
            });
            isReturn = true;
            return;
          }
    ...
    hiddenDialog: function () {
      ...
      this.triggerEvent('closeMemberDiscount');
    },
    ...
    getAppECStoreConfig: function () {
      app.getAppECStoreConfig((res) => {
        if(!this.data.themeColor){
          const theme = '';
          if(res.color_config && res.color_config.theme){
            this.setData({
              themeColor: res.color_config && res.color_config.theme
            })
          }
        }
