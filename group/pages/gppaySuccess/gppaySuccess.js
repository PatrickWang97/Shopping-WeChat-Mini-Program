...
Page({
  ...
  loadMyTeams() {
    ...
    app.sendRequest({
      ...
      success: res => {
        if(!res.data.order_info){  // 因微信接口有时会很慢，做了个递归处理
          setTimeout(() => { _this.loadMyTeams(); }, 500)
          return;
        }
        var coverList = [],
          myTeams = res.data,
  ...
  showMyTeam: function() {
    let path = `/group/pages/gpmyOrder/gpmyOrder?${this.data.orderInfo.goods_type == 14?'&index=1':''}${this.data.franchiseeId ? '&franchisee=' + this.data.franchiseeId : ''}`;
  ...
  openNewGroup: function() {
    ...
    if (myTeams.enable_status == '0') {
      if (this.data.orderInfo.goods_type == 14){  //无效预约拼团  不带拼团活动参数  
    ...
    if (this.data.orderInfo.goods_type == 14) { //预约拼团 使用特定的商品详情
      ...
      success: function(res) {
        ...
        if(status === 0 && data.length){
          ...
          if (settlement_activity && settlement_activity.item_price) {
            ...
            data[0].form_data.order_total_price = ((+order_total_price) + (+item_price)).toFixed(2);
          }
          _this.setData({
  ...
  inviteFriends: function() {
    ...
    app.sendRequest({
      ...
      success: function(res) {
        const data = res.data;
        const type = that.data.myTeams.activity_type;
        data.isGroup = 1;
        if (type == 0) {
          data.groupType = that.data.myTeams.max_user_num + '人团';
        } else {
          data.groupType = that.data.groupOptions[type].label
        }
        data.originPrice = that.data.orderInfo.goods_info[0].original_price;
