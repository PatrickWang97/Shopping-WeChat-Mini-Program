...
Page({
  ...
  onLoad: function (options) {
    const { teamtoken, orderId, franchisee, scene } = options;
    if (scene) {                  // 通过二维码进入
  ...
  getSceneData: function (scene) {
    const options = { cache_key: scene };
    app.sendRequest({
      url: '/index.php?r=appGroupBuy/GetSceneData',
      data: options,
      hideLoading: true,
      success: (res) => {
        const { status, data } = res;
        if (status === 0) {
          const { team_token, order_id, franchisee, p_id } = data;
          if (p_id) {
            app.bindUserTokenByPid(p_id);
          }
          this.setData({
  ...
  participateGroup: function(e) {
    ...
    if (that.data.goods.type != 3 && this.data.goodsInfo.goods_type == 10) { //预约拼团 需要回到商品详情 走开团流程 
      let pageUrl = `/tradeApt/pages/TYDetail/TYDetail?detail=${this.data.myTeams.goods_id}&activeId=${this.data.myTeams.activity_id}&activeType=group&team_token=${this.data.team_token}${(this.data.franchiseeId ? '&franchisee=' + this.data.franchiseeId : '')}`;
        ...
        app.sendRequest({
          ...
          success: function(res) {
            ...
            if (res.status == 0) {
              ...
              app.sendUseBehavior([{goodsId: goodsId}], 1);
            } else {
              app.showModal({
                content: res.data
              });
  ...
  turnToGoodsDetail: function() {
    ...
    if (this.data.orderInfo.goods_type == 10){ //预约拼团 使用特定的商品详情
      pageUrl = `/tradeApt/pages/TYDetail/TYDetail?detail=${this.data.myTeams.goods_id}&activeId=${this.data.myTeams.activity_id}&activeType=group${(this.data.franchiseeId ? '&franchisee=' + this.data.franchiseeId : '')}`;
    }
  ...
  openNewGroup: function() {
    ...
    if (myTeams.enable_status == '0') {
      if (this.data.goodsInfo.goods_type == 10) {  //无效预约拼团  不带拼团活动参数  
        app.turnToPage(`/tradeApt/pages/TYDetail/TYDetail?detail=${this.data.myTeams.goods_id}${(this.data.franchiseeId ? '&franchisee=' + this.data.franchiseeId : '')}`);
      } else {
    ...
    if (this.data.goodsInfo.goods_type == 10) { //预约拼团 使用特定的商品详情
      pathUrl = `/tradeApt/pages/TYDetail/TYDetail?detail=${this.data.myTeams.goods_id}&activeId=${this.data.myTeams.activity_id}&activeType=group${(this.data.franchiseeId ? '&franchisee=' + this.data.franchiseeId : '')}`;
    }
    this.saveUserFormId(function() {
      app.turnToPage(pathUrl)
    })
  ...
  showMyTeamList() {
    var _this = this;
    _this.saveUserFormId(function() {
      app.turnToPage(`/group/pages/gpmyOrder/gpmyOrder?${_this.data.goodsInfo.goods_type == 10?'&index=1':''}${_this.data.franchiseeId ? '&franchisee=' + _this.data.franchiseeId : ''}`);
    })
  },
