  ...
  dayClick: function (event) {
    ...
    this.data.memberDays.map((item) => {
      if(item.year == detail.year && item.month == detail.month && item.day == detail.day){
        let pagePath = '';
        if(item.id.length > 1){
          pagePath = '/eCommerce/pages/memberDayList/memberDayList?date=' + detail.year + '-' + detail.month + '-' + detail.day + '&franchisee=' + that.data.franchiseeId;
        }else {
          pagePath = '/eCommerce/pages/memberDayDetail/memberDayDetail?id=' + item.id[0] + '&franchisee=' + that.data.franchiseeId;
        }
