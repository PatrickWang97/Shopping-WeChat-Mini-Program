...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.appId) {
      this.selectedAppId = options.appId;
    }
    this.dataInitial();
  },
  onUnload: function () {
    ...
    for (var goodsId in this.data.additional_info_obj) {
      if (this.data.additional_info_obj[goodsId]) {
        var goods_additional_info = this.data.additional_info_obj[goodsId];
  ...
  dataInitial: function () {
    ...
    for (var i = 0; i <= goodsInfo.length - 1; i++) {
      let id = goodsInfo[i].id || goodsInfo[i].goods_id;
      if(this.goods_id_arr.indexOf(id) == -1){
        this.goods_id_arr.push(id);
  ...
  inputFormControl: function(e){
    var goodsId = e.currentTarget.dataset.goodsId,
      ...
      data = {};
    data['additional_info_obj.'+goodsId+'['+additionalIndex+'].value'] = value.trim();
    this.setData(data);
  },
  ...
  submitAdditionalInfo: function(){
    ...
    var pages = getCurrentPages();
    var prePage = pages[pages.length - 2];
    if (this.selectedAppId) { // 多店情况
      prePage.setData({
        [`mulShopsInfo.${this.selectedAppId}.additional_info`]: this.data.additional_info_obj,
        [`mulShopsInfo.${this.selectedAppId}.deliverydWrite`]: true,
      });
    } else {
      prePage.setData({
