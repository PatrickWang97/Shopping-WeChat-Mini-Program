...
Page({
  ...
  onShareAppMessage: function (e) {
    ...
    if (e.from == 'button') {
      ...
      url = '/eCommerce/pages/joinGroupDetail/joinGroupDetail?detail=' + groupInfo.goods_id + '&teamToken=' + teamToken + franchisee + (appInstance.globalData.pageShareKey ? ('&psid=' + appInstance.globalData.pageShareKey) : '');
    ...
    appInstance.showModal({
      ...
      confirm: function () {
        appInstance.sendRequest({
          ...
          success: function (res) {
            console.log(res);
            app.sendUseBehavior([{goodsId: e.target.dataset.id}],7); // 取消订单
            that.onLoad();
          }
        })
      }
