...
Page({
    ...
    app.sendRequest({
      ...
      success: function (res) {
        ...
        orderInfo.forEach(item=> {
          item.refund_goods_info.forEach(goods=> {
  ...
  showPackageInfoFn: function (e) { // 展示商品套餐详情
    let { index, status, subIndex } = e.currentTarget.dataset;
    let refund_goods_info = this.data.orderInfo[index].refund_goods_info;
    let newData = {};
    refund_goods_info[subIndex].showPackageInfo = status == 1 ? true : false;
    newData['orderInfo[' + index +'].refund_goods_info'] = refund_goods_info;
