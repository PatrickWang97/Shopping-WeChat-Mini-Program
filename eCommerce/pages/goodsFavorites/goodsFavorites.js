...
Page({
  ...
  getFavoriteGoodsList: function(){
    ...
    let parentAppId = app.getShopTypeData() && app.getShopTypeData().is_parent_shop == 1 ? app.getAppId() : '';
    if (parentAppId && app.getChainAppId() != parentAppId) {
      parentAppId = '';   // 连锁子店只展示当前店的商品
    }
    app.sendRequest({
      ...
      data: {
        ...
        parent_app_id: parentAppId, // 总店要把子店商品展示
      },
  ...
  deleteGoods: function(){
    ...
    app.sendRequest({
      ...
      success: function (res) {
        ...
        app.sendUseBehavior(goodsArr,3,2);
        _this.setData({
          favoritesList: favoritesList,
          showCheckedBox: false
        })
  ...
  goGoodsDetail: function (event) {
    let index = event.currentTarget.dataset.index;
    let goods = this.data.favoritesList[index];
    let formData = goods.form_data;
    let url = `/detailPage/pages/goodsDetail/goodsDetail?detail=${goods.id}`;
    let franchisee = goods.app_id === app.globalData.appId ? '' : '&franchisee=' + goods.app_id;
    if(formData.is_presell_goods == 1){
      url = `/presell/pages/presellDetail/presellDetail?detail=${goods.id}&activity_id=${formData.presell_activity_id}`;
    }
