...
Page({
    ...
    app.sendRequest({
      ...
      success: res => {
        if (res.status == 0) {
          let data = res.data;
          let reg=/(\d{3})\d{4}(\d{4})/
          let hidePhone = data.phone.replace(reg, "$1****$2");
          that.setData({
  ...
  infoInit: function () {
    let date = new Date();
    let userInfo = this.deepClone(app.getUserInfo());
    userInfo.phone = '';
    let userCityArr = ['', '', ''];
  ...
  bindCellphonePage: function () {
    ...
    if (this.data.cardId) {
      app.turnToPage('/default/pages/bindCellphone/bindCellphone?p=' + encodeURIComponent(this.route) + '&is_member=1', 1);
    } else {
      app.turnToPage('/default/pages/bindCellphone/bindCellphone?p=' + encodeURIComponent(this.route), 1);
    }
  },
