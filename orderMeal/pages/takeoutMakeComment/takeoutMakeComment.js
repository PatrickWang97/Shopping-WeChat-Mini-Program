...
Page({
    ...
    app.getOrderDetail({
      ...
      success: function(res){
        var goodsInfo = res.data[0].form_data.goods_info,
            is_dining = res.data[0].form_data.is_dining == 1 ? true : false,
            goods = [];
  ...
  makeComment: function(){
    ...
    for (let i = 0;i < submitData.goods.length;i++){
      if (submitData.goods[i].info.level > 2 && submitData.goods[i].info.level < 5){
        submitData.goods[i].info.level = 2;
      } else if (submitData.goods[i].info.level > 4){
        submitData.goods[i].info.level = 3;
      }
    }
    app.sendRequest({
